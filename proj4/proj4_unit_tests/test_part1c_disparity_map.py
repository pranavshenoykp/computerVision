...
from proj4_code.part1c_disparity_map import calculate_disparity_map, calculate_cost_volume
...
def test_disparity_deltafn_success():
    ...
    disp_map = calculate_disparity_map(
        im_left, im_right, block_size, ssd_similarity_measure, max_search_bound=disparity_expected + 3
    )
    ...
    assert nonzero_disp[0, 1].item() + nonzero_disp[1, 1].item() == left_c + right_c
    assert abs(nonzero_disp[0, 1].item() - nonzero_disp[1, 1].item()) == abs(left_c - right_c)
...
def test_disparity_deltafn_failure():
    ...
    # get the disparity map from the function
    disp_map = calculate_disparity_map(
        im_left, im_right, block_size, ssd_similarity_measure, max_search_bound=disparity_expected - 1
    )
...
def test_disparity_map_size():
    ...
    for block_size in (3, 5, 7, 9, 13):
        disp_map = calculate_disparity_map(im_left, im_right, block_size, ssd_similarity_measure)
...
def test_disparity_random_stereogram():
    ...
    W = 51
    disparity = 4
    im_left, im_right = generate_random_stereogram(im_size=(H, W, 3), disparity=4)
    ...
    x_lims = torch.tensor(
        [H // 2 - H // 4 - block_size // 2 - 1, H // 2 + H // 4 + block_size // 2 - 1], dtype=torch.long
    )
    y_lims = torch.tensor(
        [W // 2 - W // 4 - block_size // 2 - disparity - 1, W // 2 + W // 4 + block_size // 2 - 1], dtype=torch.long
    )
...
def test_disparity_translation_shift():
    ...
    block_size = 3
    disp_map = calculate_disparity_map(im1, im2, block_size, ssd_similarity_measure)
