...
from proj5_code.data_transforms import get_fundamental_transforms, get_fundamental_augmentation_transforms, get_all_transforms
...
def test_fundamental_transforms():
    ...
    for t in expected:
        assert t in returned_types, 'one of the expected transforms was missing: {}'.format(t)
...
def test_data_augmentation_transforms():
    ...
    tr = get_fundamental_augmentation_transforms(
        inp_size=(100, 50)
    )
...
def test_data_augmentation_with_normalization_transforms():
    ...
    tr = get_all_transforms(
        inp_size=(100, 50), pixel_mean=[0.5], pixel_std=[0.3]
    )
    expected = [transforms.Resize, transforms.ToTensor, transforms.RandomHorizontalFlip,
    transforms.Normalize, transforms.ColorJitter]
    returned_types = [type(item) for item in tr.transforms]
